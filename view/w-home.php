<?php
	...
	if (($_SESSION['usertime']) < time()){
			...
			echo "<div id= 'header'>";
				echo "<p class='logout'>";
					echo "<b>NECESITAS VOLVER A ACCEDER TU TIEMPO HA EXPIRADO : </b>"; 
				echo "(<a class='labellogin' style='color:red;' href='core/c-logout.php'>Acceder Nuevamente</a>) </p> ";  //
			echo "</div>"; //header
    }else if (!isset($_SESSION['full_name']) || !isset($_SESSION['usertime'])) {
		echo "<div id='main'  style='overflow: auto;'>";
		echo "<div id= 'header'>";
			echo "<p class='logout'>";
				echo "<b>NECESITAS VOLVER A ACCEDER TU TIEMPO HA EXPIRADO : </b>"; 
		...
		echo "<ul>";//principal
			...
			echo "<ul>";
		if(!empty($_SESSION['root']) || !empty($_SESSION['admin'])){					
